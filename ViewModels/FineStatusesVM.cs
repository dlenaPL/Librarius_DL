...
namespace Librarius_DL.ViewModels
{
    class FineStatusesVM : Utilities.AllViewModel<FineStatuses>
    {
        ...
        public override void Find()
        {
            Load();
            if (FindField == "Nazwa")
                List = new ObservableCollection<FineStatuses>(List.Where(item => item.FineStatusName != null && item.FineStatusName.StartsWith(FindTextBox, StringComparison.OrdinalIgnoreCase)));
        }
    }
}
